...
export function Products() {
  const { products, setProducts, suppliers, categories } = useData();
  ...
  const openAdd = () => {
    setEditing(null);
    setForm({ code: "", name: "", supplierId: suppliers[0]?.id || 1, categoryId: categories[0]?.id || 1, unit: "cái", price: 0, status: "Active" });
    setOpen(true);
  };
  const openEdit = (row) => {
    setEditing(row);
  ...
  const handleDelete = (id) => {
    if (window.confirm("Xóa sản phẩm này?")) setProducts(products.filter((p) => p.id !== id));
  };
            ...
            <tbody>
              {filteredProducts.map((row) => (
                <tr key={row.id}>
                  ...
                  <td className="py-3 px-4 border-b border-blue-gray-50"><Typography variant="small">{suppliers.find((s) => s.id === row.supplierId)?.name || row.supplierId}</Typography></td>
                  <td className="py-3 px-4 border-b border-blue-gray-50"><Typography variant="small">{categories.find((c) => c.id === row.categoryId)?.name || row.categoryId}</Typography></td>
