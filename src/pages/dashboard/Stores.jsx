...
export function Stores() {
  ...
  const handleDelete = (id) => {
    if (window.confirm("Xóa cửa hàng này?")) setStores(stores.filter((s) => s.id !== id));
  };
            ...
            <tbody>
              {stores.map((row) => (
                <tr key={row.id}>
                  ...
                  <td className="py-3 px-4 border-b border-blue-gray-50"><Typography variant="small">{row.phone}</Typography></td>
